...
Located at the root of your project (`/.env`), this file contains **ALL secrets** (database passwords, API keys, OAuth tokens). It is **never committed** to git.

```env
# MongoDB
MONGO_USER=admin
MONGO_PASS=your_mongo_password
DATABASE_FILE=mongodb://admin:your_mongo_password@mongodb:27017/tom_labs_db?authSource=admin
...
AI_INTERNAL_TOKEN=your-ai-token
...
> **How it works:** `docker-compose.yml` injects these as env vars → `entrypoint.sh` writes them to `.secrets_env.php` → `config.php` reads them via `getenv()` → env.json values are **never used** for secrets.

---

### <span style="color: #ff9800;">2. Application Configuration (`env_sample.json`)</span>
Refer to `env_sample.json` at the project root for the full config structure. Copy it to `/var/www/env.json` inside the container (or on bare-metal) and fill in non-secret values only.

Secrets in `env.json` are **placeholders** — they are overridden by env vars from `.env`.

```json
{
  "mongo_user": "admin",
  "mongo_pass": "REPLACE_WITH_MONGO_PASS",
  "database_file": "REPLACE_WITH_DATABASE_FILE",
  "smtp": {
    "host": "smtp.gmail.com",
    "port": 465,
    "user": "your-email@gmail.com",
    "pass": "REPLACE_WITH_SMTP_PASS"
  }
}
```
