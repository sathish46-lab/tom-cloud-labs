...
async function handleStop() {
  ...
  // OLD manual spinner injection removed
  // btn.classList.add("disabled");
  // const spinner = ...

  Dashboard.resetTerminal();
  Dashboard.appendCommand("labsctl stop essentials");
  ...
  try {
    await fetch("/api/instance/stop", {
      method: "POST",
      body: new URLSearchParams({ lab: "essentials" }),
    });
    // Note: Page likely reloads/redirects, so we don't reset loading here.
  } catch (e) {
