...
if (!empty($selectedUsername)) {
    foreach ($mysqlUsers as $mu) {
        if ($mu['mongodb_username'] === $selectedUsername) {
            $selectedUserObj = $mu;
            break;
        }
    }
    ...
    $selectedUsername = $selectedUserObj['mongodb_username'];
}

$databases = [];
if ($selectedUserObj) {
    $databases = $db->mongodb_databases->find(['mongodb_user_id' => (string)$selectedUserObj['_id']])->toArray();
}
?>
    ...
    <div class="row g-4" style="align-items: start;">
        ...
        <?php if (empty($mysqlUsers)): ?>
            <div class="col-12 text-center py-5">
                ...
                <h4 class="mt-3 text-light">No MongoDB Users Found</h4>
                <p class="text-secondary">You must create a MongoDB Server User from the Dashboard before you can create databases.</p>
                <a href="/services/mongodb" class="btn btn-primary rounded-pill mt-3 px-4 shadow-sm" style="background: linear-gradient(135deg, #6366f1, #8b5cf6); border: none;">Go to Dashboard</a>
            </div>
        <?php else: ?>
                ...
                <div class="card blur border-0 shadow-sm rounded-4 mb-4">
                    ...
                    <div class="card-body px-4 pb-4">
                        <div class="mb-3">
                            ...
                            <select id="select-db-user" class="form-select text-white shadow-none" style="background-color: rgba(0,0,0,0.2); border: 1px solid rgba(255,255,255,0.1); border-radius: 8px; cursor: pointer; color-scheme: dark;" onchange="switchMongoDBUser(this.value)">
                                <?php foreach ($mysqlUsers as $mu): ?>
                                    <option value="<?= htmlspecialchars($mu['mongodb_username']) ?>" <?= ($selectedUsername === $mu['mongodb_username']) ? 'selected' : '' ?>>
                                        <?= htmlspecialchars($mu['mongodb_username']) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
        ...
        <div class="col-lg-9 col-xl-9">
                <div class="card blur border-0 shadow-sm rounded-4 mb-4">
                    <div class="card-header bg-transparent border-bottom-0 pt-4 pb-2">
                        <h6 class="card-title fw-bold text-white mb-1" style="font-size: 1rem;">MongoDB Server Databases</h6>
                        <p class="text-secondary small mb-0">Your database will be listed below</p>
                    </div>
                    
                    <div class="card-body px-4 pb-4">
                        <div id="mongodb_db_list" class="row row-cols-1 row-cols-md-3 g-3">
                            ...
                            <?php else: ?>
                                <?php foreach ($databases as $dbObj): ?>
                                    <div class="col mongodb_db" id="mongodb_database_<?= htmlspecialchars($dbObj['db_name']) ?>">
                                        <div class="card h-100 border-0" style="background-color: rgba(0,0,0,0.2); border-radius: 12px; border: 1px solid rgba(255,255,255,0.05) !important;">
                                            <div class="card-body">
                                                ...
                                                <div class="d-grid">
                                                    <button class="btn btn-sm btn-outline-danger btn-delete" data-dbname="<?= htmlspecialchars($dbObj['db_name']) ?>" onclick="deleteMongoDBDb('<?= htmlspecialchars($dbObj['db_name']) ?>')">
                                                        Drop Database
                                                    </button>
                                                </div>
