<?php
Session::addMetaTag('<title>MariaDB Server - Tom Labs</title>');
...
foreach ($mysqlUsers as &$mUser) {
    $mUser['db_count'] = $db->mariadb_databases->countDocuments(['mariadb_user_id' => (string)$mUser['_id']]);
}
unset($mUser);
            ...
            <div class="card blur border-0 shadow-sm rounded-4 mb-4">
                ...
                <div class="card-body px-4 pb-4">
                    <div class="mb-3 d-flex justify-content-between align-items-center bg-dark bg-opacity-50 p-2 px-3 rounded border border-light border-opacity-10">
                        ...
                        <span class="text-light" style="font-size: 0.85rem;">MariaDB Server</span>
                    </div>
                    ...
                    <div class="mb-3 d-flex justify-content-between align-items-center bg-dark bg-opacity-50 p-2 px-3 rounded border border-light border-opacity-10">
                        ...
                        <div class="d-flex align-items-center">
                            ...
                            <i class='bx bx-copy text-secondary cursor-pointer hover-white' data-copy="mariadb.tomweb.in"></i>
                        </div>
                    </div>
                    <div class="mb-0 d-flex justify-content-between align-items-center bg-dark bg-opacity-50 p-2 px-3 rounded border border-light border-opacity-10">
                        ...
                        <div class="d-flex align-items-center">
                            ...
                            <i class='bx bx-copy text-secondary cursor-pointer hover-white' data-copy="3307"></i>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="col-lg-7 col-xl-8">
            <div class="card blur border-0 shadow-sm rounded-4 mb-4">
                <div class="card-header bg-transparent border-bottom-0 pt-4 pb-2 d-flex justify-content-between align-items-center">
                    ...
                    <button class="btn btn-sm btn-primary rounded-pill px-3 shadow-sm" style="background: linear-gradient(135deg, #6366f1, #8b5cf6); border: none; font-size: 0.8rem; font-weight: 600;" onclick="openAddMariaDBUserModal()"><i class='bx bx-plus'></i> Add User</button>
                </div>
                <div class="card-body px-4 pb-4">
                                        ...
                                        <div class="card-body p-3 d-flex flex-column">
                                            <div class="mb-3">
                                                ...
                                                <div class="d-flex justify-content-between align-items-center bg-dark bg-opacity-50 p-2 rounded mt-1 border border-light border-opacity-10">
                                                    <span class="text-light font-monospace" style="font-size: 0.8rem;"><?= htmlspecialchars($mu['mariadb_username']) ?></span>
                                                    <i class='bx bx-copy text-secondary cursor-pointer hover-white' data-copy="<?= htmlspecialchars($mu['mariadb_username']) ?>"></i>
                                                </div>
                                            </div>
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <div class="d-flex justify-content-between align-items-center bg-dark bg-opacity-50 p-2 rounded mt-1 border border-light border-opacity-10">
                                                    ...
                                                    <div>
                                                        <i class='bx bx-show text-secondary cursor-pointer hover-white me-2' onclick="alert('Password: <?= htmlspecialchars(base64_decode($mu['mariadb_password'])) ?>')"></i>
                                                        <i class='bx bx-copy text-secondary cursor-pointer hover-white' data-copy="<?= htmlspecialchars(base64_decode($mu['mariadb_password'])) ?>"></i>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div class="d-flex flex-column gap-2 mt-auto">
                                                <a href="/services/mariadb/database?user=<?= htmlspecialchars($mu['mariadb_username']) ?>" class="btn btn-sm btn-primary rounded-pill w-100" style="background: linear-gradient(135deg, #6366f1, #8b5cf6); border: none; font-size: 0.8rem; font-weight: 600;">Manage Database</a>
                                                <button class="btn btn-sm btn-outline-danger rounded-pill w-100" style="font-size: 0.8rem; font-weight: 600;" onclick="deleteMariaDBUser('<?= htmlspecialchars($mu['mariadb_username']) ?>')">Delete User</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content" style="background: #1e293b; border: 1px solid rgba(255,255,255,0.1); border-radius: 16px;">
      <div class="modal-header border-bottom-0 pb-0">
        <h5 class="modal-title text-white fw-bold">Add MariaDB User</h5>
      ...
      <div class="modal-footer border-top-0 pt-0">
        ...
        <button type="button" class="btn btn-primary rounded-pill px-4" style="background: linear-gradient(135deg, #6366f1, #8b5cf6); border: none;" onclick="submitCreateMariaDBUser()" id="btn-submit-user">Add User</button>
      </div>
    </div>
  </div>
</div>
...
ServiceManager.init({
    type: 'mariadb',
    apiBase: '/api/services/mariadb',
    entityLabel: 'Database',
    gridId: 'mariadb_db_list',
    hasEntities: true,
    entityNameKey: 'db_name',
    userKey: 'mariadb_username',
    entityCreateEndpoint: 'db_create',
    entityDeleteEndpoint: 'db_delete',
    redirectBase: '/services/mariadb',
    exportPrefix: 'MariaDB'
});
</script>
