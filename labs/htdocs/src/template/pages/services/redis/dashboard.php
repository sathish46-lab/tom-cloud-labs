...
// Fetch the Redis users
$mysqlUsers = $db->redis_users->find(['user_id' => $user->getUserId()])->toArray();

// For each user, get the number of databases they have
foreach ($mysqlUsers as &$mUser) {
    $mUser['redis_count'] = $db->redis_databases->countDocuments(['redis_user_id' => (string)$mUser['_id']]);
}
unset($mUser);
...
require_once __DIR__ . '/partials/redis_header.php';
?>
...
<div class="container-fluid px-2 bg-transparent">
    ...
    <div class="row g-4" style="align-items: start;">
            ...
            <div class="card blur border-0 shadow-sm rounded-4 mb-4">
                ...
                <div class="card-body px-4 pb-4">
                    <div class="mb-3 d-flex justify-content-between align-items-center bg-dark bg-opacity-50 p-2 px-3 rounded border border-light border-opacity-10">
                        ...
                        <span class="text-light" style="font-size: 0.85rem;">Redis Server</span>
                    </div>
        ...
        <!-- Right Column: Redis Users -->
        <div class="col-lg-7 col-xl-8">
            <div class="card blur border-0 shadow-sm rounded-4 mb-4">
                <div class="card-header bg-transparent border-bottom-0 pt-4 pb-2 d-flex justify-content-between align-items-center">
                    <div>
                        <h5 class="card-title fw-bold text-white mb-0 d-inline-block me-2" style="font-size: 1.1rem;">Redis Server Users</h5>
                        <span class="text-secondary" style="font-size: 0.8rem;">(Maximum 5 users)</span>
                    </div>
                    <button class="btn btn-sm btn-primary rounded-pill px-3 shadow-sm" style="background: linear-gradient(135deg, #6366f1, #8b5cf6); border: none; font-size: 0.8rem; font-weight: 600;" onclick="openAddRedisUserModal()"><i class='bx bx-plus'></i> Add User</button>
                </div>
                <div class="card-body px-4 pb-4">
                                    ...
                                    <div class="card border-0 shadow-sm position-relative overflow-hidden" style="background-color: rgba(0,0,0,0.2); border-radius: 12px; border: 1px solid rgba(255,255,255,0.05) !important;">
                                        ...
                                        <div class="card-body p-3 d-flex flex-column">
                                            ...
                                            <div class="d-flex flex-column gap-2 mt-auto">
                                                ...
                                                <button class="btn btn-sm btn-outline-danger rounded-pill w-100" style="font-size: 0.8rem; font-weight: 600;" onclick="deleteRedisUser('<?= htmlspecialchars($mu['redis_username']) ?>')">Delete User</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content" style="background: #1e293b; border: 1px solid rgba(255,255,255,0.1); border-radius: 16px;">
      <div class="modal-header border-bottom-0 pb-0">
        <h5 class="modal-title text-white fw-bold">Add Redis User</h5>
      ...
      <div class="modal-footer border-top-0 pt-0">
        ...
        <button type="button" class="btn btn-primary rounded-pill px-4" style="background: linear-gradient(135deg, #6366f1, #8b5cf6); border: none;" onclick="submitCreateRedisUser()" id="btn-submit-user">Add User</button>
      </div>
    </div>
  </div>
</div>
