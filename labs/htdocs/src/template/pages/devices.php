...
<div class="lab-header-section mb-4 d-flex justify-content-between align-items-center">
    <div>
        <h1 class="fw-bold theme-text m-0">Lab Environment</h1>
        <p class="text-secondary opacity-75 small">Live status monitoring and secure tunnel management.</p>
    </div>
    <button class="btn btn-success fw-bold px-4 rounded-pill shadow-sm" data-coreui-toggle="modal"
        data-coreui-target="#addDeviceModal"><i class="bx bx-plus"></i> Add Device</button>
</div>
        ...
        <div class="card shadow-lg rounded-4 overflow-hidden border-0">
            <div class="card-body p-4">
                <div class="d-flex justify-content-between align-items-start mb-2">
                    <h5 class="fw-bold m-0 theme-text text-truncate" style="text-transform: none;">
                        <?= htmlspecialchars($device['device_name']) ?></h5>
                    <div class="dropdown">
                        <button class="btn btn-link text-secondary p-0 border-0" data-coreui-toggle="dropdown"><i
                                class="bx bx-dots-vertical-rounded fs-4"></i></button>
                        <ul class="dropdown-menu dropdown-menu-end p-2 rounded-3 shadow-lg border-0">
                            <li><a class="dropdown-item rounded-2" href="javascript:void(0)"
                                    onclick="showConfig('<?= $device['device_name'] ?>', '<?= $device['assigned_ip'] ?>', '<?= $device['private_key'] ?? '' ?>')"><i
                                        class='bx bx-qr-scan me-2 text-primary'></i>Show Configuration</a></li>
                            <li><a class="dropdown-item rounded-2" href="javascript:void(0)"
                                    onclick="downloadTunnel('<?= $device['device_name'] ?>', '<?= $device['assigned_ip'] ?>', '<?= $device['private_key'] ?? '' ?>')"><i
                                        class='bx bx-download me-2 text-info'></i>Download .conf</a></li>
                            <li>
                                <hr class="dropdown-divider opacity-10">
                            </li>
                            <li><a class="dropdown-item rounded-2 text-danger fw-bold" href="javascript:void(0)"
                                    onclick="deleteDevice('<?= $dbId ?>', '<?= $device['public_key'] ?>')"><i
                                        class='bx bx-trash me-2'></i>Delete Device</a></li>
                        </ul>
                    </div>
                </div>
                <div class="d-flex gap-2 mb-3">
                    <span
                        class="badge rounded-pill bg-primary px-2 py-1 "><?= ucfirst(strtolower($device['device_type'] ?? 'Mobile')) ?></span>
                    <span class="badge rounded-pill small status-pill bg-secondary px-2 py-1"><?= $displayStatus ?></span>
                </div>
                <div class="small text-secondary stats-area">
                    <div class="d-flex justify-content-between mb-1"><span>Device IP:</span> <span
                            class="theme-text font-monospace"><?= $device['assigned_ip'] ?></span></div>
                    <div class="d-flex justify-content-between mb-1"><span>Origin:</span> <span
                            class="origin-val theme-text"><?= $device['origin_ip'] ?></span></div>
                    <div class="d-flex justify-content-between mb-1"><span>Received:</span> <span
                            class="rx-val text-info fw-bold"><?= $device['rx'] ?></span></div>
                    <div class="d-flex justify-content-between"><span>Sent:</span> <span
                            class="tx-val text-warning fw-bold"><?= $device['tx'] ?></span></div>
                </div>
            </div>
        </div>
    </div>
