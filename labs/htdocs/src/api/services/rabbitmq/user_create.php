...
try {
    // Limit to 5 RabbitMQ users per account
    $userCount = $db->rabbitmq_users->countDocuments([
        'user_id' => $user->getUserId(),
        'email' => $user->getEmail()
    ]);
    if ($userCount >= 5) {
        echo json_encode(['success' => false, 'error' => 'You have reached the maximum limit of 5 RabbitMQ users.']);
        exit;
    }
    ...
    if (!$manager->createUser($dbUser, $password)) {
        throw new Exception("Failed to create RabbitMQ user. The username '{$dbUser}' might already be taken globally.");
    }
    ...
    $dbRecord = [
        'user_id' => $user->getUserId(),
        'email' => $user->getEmail(),
        'rabbitmq_username' => $dbUser,
        'rabbitmq_password' => base64_encode($password),
        'created_at' => new MongoDB\BSON\UTCDateTime()
    ];
    
    $db->rabbitmq_users->insertOne($dbRecord);

    echo json_encode([
        'success' => true,
        'message' => 'RabbitMQ user created successfully.'
    ]);
