...
$mysqlUsername = $data['postgresql_username'] ?? '';
$dbNameRaw = trim($data['db_name'] ?? '');
$collation = trim($data['collation'] ?? 'UTF8');

if (empty($mysqlUsername) || empty($dbNameRaw)) {
    echo json_encode(['success' => false, 'error' => 'PostgreSQL username and database name are required.']);
    exit;
}
...
try {
    // 1. Verify ownership of the parent PostgreSQL user
    ...
    // 2. Limit to 10 databases per PostgreSQL user
    $dbCount = $db->postgresql_databases->countDocuments([
    ...
    // If we use exact, there's a high chance of collisions globally. Let's prefix with their PostgreSQL username.
    ...
    $db->postgresql_databases->insertOne($dbRecord);
