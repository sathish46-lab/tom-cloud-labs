...
try {
    // Limit to 5 MariaDB users per account
    $userCount = $db->mariadb_users->countDocuments([
        'user_id' => $user->getUserId(),
        'email' => $user->getEmail()
    ]);
    if ($userCount >= 5) {
        echo json_encode(['success' => false, 'error' => 'You have reached the maximum limit of 5 MariaDB users.']);
        exit;
    }

    $manager = new MariaDbManager();

    // 1. Create MariaDB User
    if (!$manager->createUser($dbUser, $password)) {
        throw new Exception("Failed to create MariaDB user. The username '{$dbUser}' might already be taken globally.");
    }
    ...
    $dbRecord = [
        ...
        'mariadb_password' => base64_encode($password),
        'created_at' => new MongoDB\BSON\UTCDateTime()
    ];
    
    $db->mariadb_users->insertOne($dbRecord);

    echo json_encode([
        'success' => true,
        'message' => 'MariaDB user created successfully.'
    ]);
