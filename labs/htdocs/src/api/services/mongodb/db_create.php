...
$mysqlUsername = $data['mongodb_username'] ?? '';
$dbNameRaw = trim($data['db_name'] ?? '');
$collation = trim($data['collation'] ?? 'utf8mb4_0900_ai_ci');

if (empty($mysqlUsername) || empty($dbNameRaw)) {
    echo json_encode(['success' => false, 'error' => 'MongoDB username and database name are required.']);
    exit;
}
...
try {
    // 1. Verify ownership of the parent MongoDB user
    ...
    // 2. Limit to 10 databases per MongoDB user
    $dbCount = $db->mongodb_databases->countDocuments([
    ...
    // If we use exact, there's a high chance of collisions globally. Let's prefix with their MongoDB username.
    ...
    $db->mongodb_databases->insertOne($dbRecord);
