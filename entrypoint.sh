...
if [ -f "/var/www/env.json" ]; then
    ...
    # NOTE: Only inject if not already set via environment variable
    if [ -f /etc/wireguard/publickey ] && [ -z "${WIREGUARD_PUBLIC_KEY:-}" ]; then
...
# 5.1 Export Docker env vars to a PHP bootstrap file (for Apache/PHP processes)
echo "[INFO] Writing secrets bootstrap file for PHP..."
SECRETS_FILE="/var/www/labs/htdocs/src/.secrets_env.php"
cat > "$SECRETS_FILE" << 'SECRETEOF'
<?php
/**
 * Auto-generated by entrypoint.sh — DO NOT EDIT.
 * Sets environment variables from Docker for PHP processes.
 * These override env.json values in get_config().
 */
SECRETEOF

# Append each env var (use putenv with fallback to current value)
for var_name in DATABASE_FILE MONGO_PASS RABBITMQ_PASS API_SECRET AI_API_KEY AI_INTERNAL_TOKEN SMTP_PASS S3_ACCESS_KEY S3_SECRET_KEY GOOGLE_CLIENT_ID GOOGLE_CLIENT_SECRET GITLAB_CLIENT_ID GITLAB_CLIENT_SECRET WIREGUARD_PUBLIC_KEY; do
    eval "var_val=\${$var_name:-}"
    if [ -n "$var_val" ]; then
        echo "putenv('${var_name}=${var_val}');" >> "$SECRETS_FILE"
    fi
done

chmod 640 "$SECRETS_FILE"
chown www-data:www-data "$SECRETS_FILE"
echo "[INFO] Secrets bootstrap file created ($SECRETS_FILE)"
